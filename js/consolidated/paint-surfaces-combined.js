...
(function() {
  ...
  function createPaintCardEnhanced(projectType, index) {
    ...
    // Debug logging
    console.log('Creating brand dropdown - window.bitrixProducts:', window.bitrixProducts);
    console.log('Number of products:', window.bitrixProducts ? window.bitrixProducts.length : 0);
    
    // Extract unique brands
    let uniqueBrands = [];
    if (window.bitrixProducts && window.bitrixProducts.length > 0) {
      const allBrands = window.bitrixProducts.map(p => p.brand).filter(Boolean);
    ...
    if (projectType === 'interior') {
      ...
      console.log('Brands after filtering UCI for interior:', uniqueBrands);
    }
    
    let brandOptions = '<option value="">Select a brand...</option>';
    uniqueBrands.forEach(brand => {
      // Don't abbreviate brand names - they're not that long
      brandOptions += `<option value="${brand}" title="${brand}">${brand}</option>`;
    });
    ...
    // Ensure brand dropdown is properly populated after DOM is ready
    setTimeout(() => {
      console.log('Checking brand dropdown after card creation');
      updateBrandAvailability(paintId, paintSelection.finish || '', projectType);
    }, 100);
  ...
  function updateBrandAvailability(paintId, finish, projectType) {
    ...
    // Debug logging
    console.log('updateBrandAvailability called with:', { paintId, finish, projectType });
    console.log('Current window.bitrixProducts:', window.bitrixProducts);
    
    // First, ensure we have all brands in the dropdown
    ...
    window.bitrixProducts.forEach(product => {
      ...
      if (scopeMatch && product.brand) {
        allBrands.add(product.brand);
      }
    });
    
    console.log('All brands for project type:', Array.from(allBrands));
    
    // Get current value before rebuilding
    const currentBrand = brandSelect.value;
    
    // Check if we need to rebuild the options (if brands are missing)
    const existingBrands = new Set(Array.from(brandSelect.options).map(opt => opt.value).filter(v => v !== ''));
    console.log('Existing brands in dropdown:', Array.from(existingBrands));
    
    let needsRebuild = false;
    for (const brand of allBrands) {
      if (!existingBrands.has(brand)) {
        needsRebuild = true;
        console.log('Missing brand in dropdown:', brand);
        break;
      }
    }
    ...
    if (needsRebuild) {
      console.log('Rebuilding brand dropdown with all brands');
      let brandOptions = '<option value="">Select a brand...</option>';
    ...
    // Now get available brands based on finish
    ...
    window.bitrixProducts.forEach(product => {
      ...
      // Handle finish matching with compatibility for old finish names
      let finishMatch = !finish; // If no finish selected, all brands are available
    ...
    console.log('Available brands for finish', finish, ':', Array.from(availableBrands));
    ...
    // Restore current brand if it's still available
    if (currentBrand && availableBrands.has(currentBrand)) {
      brandSelect.value = currentBrand;
    } else if (currentBrand && !availableBrands.has(currentBrand)) {
